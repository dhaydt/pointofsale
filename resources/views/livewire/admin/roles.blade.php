        ...
        <div class="card-header">
            ...
            <div class="card-toolbar">
                <button type="button" wire:click.prevent="$emit('onClickAdd')"
                    ...
                    data-bs-placement="top" title="Tambah Role">
            ...
            <div class="row justify-content-between">
                ...
                <div class="col-md-6 text-end">
                    <button type="button" class="btn btn-light-success btn-sm" wire:click="$emit('onClickRefresh')" data-bs-toggle="tooltip"
                        data-bs-placement="top" title="Refresh Data Role">
                ...
                <table id="id" class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-sm text-dark font-weight-bolder opacity-75 text-center">Nama Role
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="align-middle text-center text-capitalize">{{ $item->role}}</td>
                            <td class="align-middle text-center">
                                @php($hak = json_decode($item->hak_akses))
                                @foreach ($hak as $s)
                                <span class="badge badge-success text-capitalize">{{ $s }}</span>
                                @endforeach
                            </td>
                            <td class="align-middle text-center">
                                <div class="btn-group btn-group-sm" role="group">
                                    ...
                                    <button type="button" wire:click.prevent="$emit('onClickDelete', `{{ $item->id }}`)"
                                        ...
                                        data-bs-placement="top" title="Hapus data {{ $item->role }}"><i
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Masukan Hak Akses</h5>
                ...
                <form wire:submit.prevent="save" id="form_add">
                    <div class="modal-body">
                        ...
                        <div class="mb-10" wire:ignore>
                            ...
                            <select id="hak_akses" class="form-select" aria-label="Select example" multiple name="hak_akses" wire:model="hak_akses">
                                @foreach ($list_hak as $lc)
                                <option value="{{ $lc }}" class="text-capitalize">{{ $lc }}</option>
                                @endforeach
                            </select>
                            @error('hak_akses')
                            <small class="text-danger">{{ $message }}</small>
            ...
            <div class="modal-content">
                ...
                <form wire:submit.prevent="update" id="form_update">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-10" id="update_akses">
                            <label for="hakAkses" class="required form-label">Hak Akses</label>
                            <select id="hak_akses_update" class="form-select" aria-label="Select example" multiple name="hak_akses" data-dropdown-parent="#modal_update"
                                wire:model="hak_akses">
                                @foreach ($list_hak as $lc)
                                <option wire:key="{{ $lc }}" value="{{ $lc }}" class="text-capitalize" @if (in_array($lc, $hak_akses)) {{'selected'}} @endif>{{ $lc }}</option>
                                @endforeach
                            </select>
                            @error('hak_akses')
                            <small class="text-danger">{{ $message }}</small>
...
<script>
    ...
    window.addEventListener('contentChange', event => {
        ...
        Livewire.on("finishRoles", (status, message) => {
            alertMessage(status, message)
        })

        Livewire.on('onClickRefresh', (id) => {
            Livewire.emit('refreshRoles')
        ...
        Livewire.on('onClickUpdate', (item) => {
            Livewire.emit('setDataRole', item);
            $('#modal_update').modal('show')
            $('#update_akses').attr('wire:ignore')
        })

        Livewire.on('onClickDelete', async (id) => {
            const response = await alertHapus('Warning !!!', 'Anda yakin ingin menghapus data role ini?')
